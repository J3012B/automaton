...
import type {
  ...
  export function createInferenceClient(
	...
	// Use dedicated inference credentials when provided, otherwise fall back to Conway credentials.
	// OpenAI-compatible APIs (including OpenAI itself) require the "Bearer " prefix.
	const effectiveApiUrl = inferenceApiUrl || apiUrl;
	const effectiveAuthHeader = inferenceApiKey
	  ? `Bearer ${inferenceApiKey}`
	  : apiKey;
	...
	const chat = async (
	  ...
	  const resp = await fetch(`${effectiveApiUrl}/v1/chat/completions`, {
		method: "POST",
		headers: {
