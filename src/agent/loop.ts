	...
	while (running) {
		try {
			...
			if (
				...
				if (consecutiveTextOnlyTurns >= 2) {
					// Agent has produced multiple text-only responses in a row — truly idle.
					...
					// Agent described plans but didn't act. Prompt it to follow through.
					log(config, "[CONTINUE] Agent has unexecuted plans. Prompting for action.");
					pendingInput = {
						content:
							"You just described your next steps. Please proceed with the first concrete action now by calling the appropriate tool, or call the sleep tool if there is genuinely nothing to do right now.",
						source: "system",
					};
				}
			} else {
